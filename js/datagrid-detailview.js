var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		for(var i=0; i<rows.length; i++) {
			...
			table.push('<tr id="' + rowId + '" datagrid-row-index="' + i + '" ' + cls + ' ' + style + '>');//where will expand row no.
		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				if (col.checkbox){
					...
					cc.push('<span class="datagrid-row-expander datagrid-row-expand" style="display:inline-block;width:38px;height:16px;cursor:pointer;" />');
					//cc.push('</div>');
	...
	onBeforeRender: function(target){
		...
		if (!hasExpander){
			...
			var td = $('<td rowspan="'+opts.frozenColumns.length+'"><div class="datagrid-header-expander" style="width:38px; font-weight:bold; color:#FAF32E">More</div></td>');
			if ($('tr',t).length == 0){
