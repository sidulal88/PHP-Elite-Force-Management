...
class Administrative_Orders
{
    ...
    function __construct()
	{
	  ...
	  self::$tableName = TABLE_PREFIX.'administrative_orders';
	  self::$PrimeryKey = DataProcess::getPrimKey(self::$tableName);
	  //self::$ForeignKey = 'sub_under';
	  self::$extendent_folder_path = 'administrative/';
	}
	...
	public function get_ui_grid($condition='', $offset, $rows)
	{
		$sql = "SELECT
						*
				FROM
					 ipb_administrative_orders
				WHERE 
					ipb_administrative_orders.status='active' $condition 
				ORDER BY
					date DESC
				LIMIT
					$offset,$rows";
