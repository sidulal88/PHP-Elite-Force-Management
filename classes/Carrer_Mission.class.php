...
class Carrer_Mission
{
	...
	public function add()
	{
		...
		$_POST['joining_date'] = Common::converToMysqlDate($_POST['joining_date']);
	...
	public function get_ui_grid($condition='', $offset, $rows)
	{
		$sql = "SELECT
				...
				FROM
					 ipb_un_mission
				WHERE 
					ipb_un_mission.status='Active' $condition 
				LIMIT
					$offset,$rows";
		$result = Database::query($sql, 2, __FILE__, __LINE__);
		return $result;
