...
function grid_view() {
    ...
    $('#dg').datagrid({
        ...
        pagePosition: 'bottom',
        idField: 'district_id',
        url: '../ajax_admin_grid.php?gname=district_list',
        columns: [[
				 {field: 'distrcit_name', title: 'District name', styler:cellStyler},
...
function editRecord(){
            ...
            if (row){
                ...
                $('#fm').form('load',row);
                url = 'ajax_record_process.php?primery_key='+ row.district_id +'&mode=save';
            }
}
		...
		function destroyRecord(){
            ...
            if (row){
                $.messager.confirm('Confirm','Are you sure you want to destroy this Record?',function(r){
                    if (r){
                        $.post('ajax_record_process.php',{primery_key:row.district_id},function(result){
                            if (result.success){
