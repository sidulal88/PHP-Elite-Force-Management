...
function grid_view() {
    ...
    $('#dg').datagrid({
        ...
        pagePosition: 'bottom',
        idField: 'vacancy_id',
        url: '../ajax_directory_grid.php?gname=vacancy_list',
        columns: [[
                {field: 'title', title: 'Title', styler:cellStyler},
				{field: 'date', title: 'Date', styler:cellStyler},
				{field: 'web_link', title: 'Web Link', styler:cellStyler},
		...
		function destroyRecord(){
            ...
            if (row){
                $.messager.confirm('Confirm','Are you sure you want to destroy this Record?',function(r){
                    if (r){
                        $.post('ajax_record_process.php',{primery_key:row.vacancy_id},function(result){
                            if (result.success){
...
function editRecord() {
	var row = $('#dg').datagrid('getSelected');
    location.replace('record_edit.php?vacancy_id=' + row.vacancy_id);
}
