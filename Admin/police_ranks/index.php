...
function grid_view() {
    ...
    $('#dg').datagrid({
        ...
        pagePosition: 'bottom',
        idField: 'rank_id',
        url: '../ajax_admin_grid.php?gname=rank_list',
        columns: [[
...
function editRecord(){
            ...
            if (row){
                ...
                $('#fm').form('load',row);
                url = 'ajax_record_process.php?primery_key='+ row.rank_id +'&mode=save';
            }
}
		...
		function destroyRecord(){
            ...
            if (row){
                $.messager.confirm('Confirm','Are you sure you want to destroy this Record?',function(r){
                    if (r){
                        $.post('ajax_record_process.php',{primery_key:row.rank_id},function(result){
                            if (result.success){
